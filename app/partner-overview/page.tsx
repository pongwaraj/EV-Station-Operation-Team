  ...
  return (
    <main className="shell partner-overview-shell">
      ...
      {data?.kpis && !loading && !error && <>
        <section className="grid kpi-grid partner-overview-kpi-grid"><article className="card"><p className="card-label">จำนวนการใช้บริการ</p><p className="kpi-value">{formatNumber(data.kpis.sessions)}<small> ครั้ง</small></p><p className="hint">เข้าเกณฑ์วิเคราะห์พฤติกรรม {formatNumber(data.kpis.meaningfulSessions)} ครั้ง</p></article><article className="card"><p className="card-label">ผู้ใช้งานทั้งหมดที่ระบุได้</p><p className="kpi-value">{formatNumber(data.kpis.uniqueCustomers)}<small> คน</small></p><p className="hint">รวมผู้ใช้งานจากรายการชาร์จทั้งหมด</p></article><article className="card"><p className="card-label">ปริมาณการชาร์จรวม</p><p className="kpi-value">{formatNumber(data.kpis.energyKwh, 2)}<small> kWh</small></p><p className="hint">เฉลี่ย {formatNumber(data.kpis.averageKwhPerSession, 2)} kWh/ครั้ง</p></article><article className="card"><p className="card-label">ระยะเวลาชาร์จเฉลี่ย</p><p className="kpi-value">{formatNumber(data.kpis.averageDurationMinutes, 1)}<small> นาที</small></p><p className="hint">ระยะเวลาที่ลูกค้าใช้บริการโดยเฉลี่ย</p></article></section>
        <section className="panel partner-decision-panel"><p className="section-label">STATION HIGHLIGHTS</p><h2>ภาพรวมการใช้งานในช่วงที่เลือก</h2><div className="partner-decision-grid"><div><span>จำนวนครั้งที่ใช้บริการ</span><strong>{formatNumber(data.kpis.sessions)} ครั้ง</strong><small>รวมรายการชาร์จในช่วงวันที่เลือก</small></div><div><span>ผู้ใช้งานทั้งหมดที่ระบุได้</span><strong>{formatNumber(data.kpis.uniqueCustomers)} คน</strong><small>รวมจากรายการชาร์จทั้งหมด</small></div><div><span>พลังงานที่ให้บริการ</span><strong>{formatNumber(data.kpis.energyKwh, 2)} kWh</strong><small>ปริมาณการชาร์จรวมของสถานี</small></div><div><span>เวลาที่ใช้บริการ</span><strong>{formatNumber(data.kpis.averageDurationMinutes, 1)} นาที/ครั้ง</strong><small>ระยะเวลาชาร์จเฉลี่ย</small></div></div></section>
