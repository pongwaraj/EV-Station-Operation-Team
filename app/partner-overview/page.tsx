...
export default function PartnerOverviewPage() {
  ...
  return (
    <main className="shell partner-overview-shell">
      <section className="hero compact-hero"><p className="eyebrow">PARTNER VIEW · OVERVIEW</p><h1>Partner Overview</h1><p className="lede">ภาพรวมการใช้งานสถานี Meta Mall สำหรับผู้ให้เช่าพื้นที่ แสดง traffic, หน่วยชาร์จ และ customer behavior ระดับรวม</p></section>
      <section className="partner-safe-banner"><span className="partner-safe-icon">✓</span><div><strong>Partner workspace</strong><p>ภาพรวมการใช้งานและแนวโน้มลูกค้าสำหรับใช้ประกอบการวางแผนกิจกรรมในพื้นที่</p></div></section>
      ...
      {data?.kpis && !loading && !error && <>
        ...
        <section className="panel partner-overview-trend-panel"><div className="decision-heading"><div><p className="section-label">USAGE TREND</p><h2>หน่วยชาร์จรายวัน</h2></div><span className="period-label">แนวโน้มการใช้บริการ</span></div><div className="partner-overview-trend-chart">{(data.trend ?? []).map((row) => <div className="partner-overview-trend-column" key={row.date} title={`${row.date} · ${formatNumber(row.energyKwh, 2)} kWh`}><strong>{formatNumber(row.energyKwh, 0)}</strong><i style={{ height: `${Math.max(2, row.energyKwh / maxEnergy * 100)}%` }} /><span>{row.date.slice(8)}</span></div>)}</div><p className="chart-footnote">แสดงปริมาณการชาร์จรวมรายวันเพื่อดู demand และแนวโน้มการใช้พื้นที่</p></section>
        <section className="panel partner-method-panel"><p className="section-label">นิยามและสถานะข้อมูล</p><p>{data.methodology?.energy}</p><p className="chart-footnote">{data.methodology?.note} · นำเข้าข้อมูลล่าสุด {formatDateTime(data.imports?.lastImportAt)}</p></section>
      </>}
    </main>
  );
}
