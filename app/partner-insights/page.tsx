    ...
    <main className="shell partner-insights-shell">
      ...
      {data?.kpis && !loading && !error && (
        <>
          ...
          <section className="panel partner-crm-panel"><div className="decision-heading"><div><p className="section-label">CRM SEGMENTS</p><h2>กลุ่มลูกค้าสำหรับวางแผนกิจกรรม</h2></div><span className="period-label">อิง meaningful session</span></div><div className="partner-crm-grid"><div><span>ลูกค้าที่เริ่มใช้ในช่วงข้อมูล</span><strong>{formatNumber(data.crmSegments?.newCustomers ?? 0)} คน</strong><small>เหมาะกับกิจกรรมสร้างการทดลองใช้</small></div><div><span>ลูกค้าที่ใช้ซ้ำในช่วงนี้</span><strong>{formatNumber(data.crmSegments?.returningCustomers ?? 0)} คน</strong><small>เหมาะกับกิจกรรมรักษาความถี่</small></div><div><span>ลูกค้าที่กลับมาหลังหายไป</span><strong>{formatNumber(data.crmSegments?.reactivatedCustomers ?? 0)} คน</strong><small>ห่างจากครั้งก่อนอย่างน้อย {formatNumber(data.crmSegments?.reactivationGapDays ?? 14)} วัน</small></div></div></section>
