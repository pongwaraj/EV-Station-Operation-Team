    ...
    <main className="shell partner-insights-shell">
      ...
      {data?.kpis && !loading && !error && (
        <>
          ...
          <section className="panel partner-pattern-panel"><div className="decision-heading"><div><p className="section-label">WEEKDAY PATTERN</p><h2>จำนวนการชาร์จเฉลี่ยรายวัน จันทร์–อาทิตย์</h2></div><span className="period-label">ใช้เลือกวันที่ทำ campaign</span></div><div className="partner-weekday-chart">{(data.weekday ?? []).map((row) => <div className="partner-weekday-column" key={row.label} title={`${row.label} · เฉลี่ย ${formatNumber(row.averageSessions, 1)} ครั้ง/วัน`}><strong>{formatNumber(row.averageSessions, 1)}</strong><i style={{ height: `${Math.max(4, row.averageSessions / maxWeekday * 100)}%` }} /><span>{row.label.slice(0, 3)}</span></div>)}</div></section>

          <section className="panel partner-pattern-panel"><div className="decision-heading"><div><p className="section-label">HOURLY PATTERN</p><h2>จำนวนการชาร์จตามช่วงเวลา</h2></div><span className="period-label">แสดง session ที่เริ่มในแต่ละชั่วโมง</span></div><div className="partner-hourly-chart">{(data.hourly ?? []).map((row) => <div className="partner-hourly-column" key={row.hour} title={`${hourLabel(row.hour)} · ${formatNumber(row.sessions)} ครั้ง · เฉลี่ย ${formatNumber(row.averageSessions, 1)} ครั้ง/วัน`}><strong>{row.sessions > 0 ? formatNumber(row.averageSessions, 1) : ""}</strong><i style={{ height: `${Math.max(2, row.sessions / maxHourly * 100)}%` }} /><span>{row.hour % 2 === 0 ? String(row.hour).padStart(2, "0") : ""}</span></div>)}</div></section>
